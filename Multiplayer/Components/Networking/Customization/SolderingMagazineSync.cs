...
internal static class SolderingMagazineSync
{
    ...
    {
        GameObject spool = tool?.magazine != null && tool.magazine.Capacity > 0 ? tool.magazine[0] : null;
        if (spool == null)
            return 0;
    ...
    private static void OnServerMagazine(NetworkServer server, SolderingMagazinePacket packet, IPlayer sender)
    {
        ...
        if (packet.SpoolItemNetId == 0)
        {
            if (!ReplaceFullSpoolWithSpent(tool, out canonicalSpool))
                return;
            packet = new SolderingMagazinePacket
            {
                ToolItemNetId = packet.ToolItemNetId,
                SpoolItemNetId = canonicalSpool.NetId,
            };
        }
        else if (!NetworkedItem.TryGet(packet.SpoolItemNetId, out canonicalSpool) || canonicalSpool?.Item == null ||
    ...
    {
        ItemMagazine magazine = tool?.magazine;
        if (magazine == null)
            return false;
        ...
        if (magazine[0] == spool.Item.gameObject)
            return true;
        if (!magazine.ValidItem(spool.Item.gameObject))
            return false;
    ...
    private static bool ReplaceFullSpoolWithSpent(GadgetSolderingTool tool, out NetworkedItem replacement)
    {
        ...
        if (current == null || ammo == null || ammo.isSpent || tool.emptyCoilItemPrefab == null || tool.reelInteractionPoint == null)
            return false;
        ...
        using (CustomizationSyncScope.Remote(rootAction: true))
        {
            ...
            GameObject spent = Object.Instantiate(tool.emptyCoilItemPrefab,
                tool.reelInteractionPoint.transform.position,
                tool.reelInteractionPoint.transform.rotation);
            tool.UpdateEmptySpoolItemParams(spent);
            if (!magazine.AddItem(spent, 0))
            {
                Object.Destroy(spent);
                return false;
            }
    ...
    private static void EnsureKnown(NetworkServer server, ServerPlayer player, NetworkedItem item)
    {
    ...
    private static bool TryGetToolNetId(GadgetSolderingTool tool, out ushort netId)
    {
        ...
        if (item == null || !NetworkedItem.TryGetNetworkedItem(item, out NetworkedItem networked) || networked.NetId == 0)
            return false;
