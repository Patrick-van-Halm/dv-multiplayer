...
public static class GadgetTrackedValueRegistry
{
        ...
        for (int componentIndex = 0; componentIndex < drillables.Length; componentIndex++)
        {
            ...
            for (int pointIndex = 0; pointIndex < drillable.MountPointCount; pointIndex++)
            {
                ...
                item.RegisterTrackedValue(
                    ...
                    value => drillable.GetMountPoint(pi).IsOnGlass = value);
            }
        }
    }
    ...
    private static void RegisterRoadrunner(NetworkedItem item, GadgetRoadrunner roadRunner)
    {
        ...
        if (target != null && target.FieldType == typeof(float))
            item.RegisterTrackedValue("gadget.roadrunner.target", () => (float)target.GetValue(roadRunner),
                value => target.SetValue(roadRunner, value));
        if (countup != null && countup.FieldType == typeof(float))
            item.RegisterTrackedValue("gadget.roadrunner.countup", () => (float)countup.GetValue(roadRunner),
                value => countup.SetValue(roadRunner, value));
    }
